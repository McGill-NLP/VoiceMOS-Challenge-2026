...
"""
Fine-tuning for VoiceMOS 2026 Track 3 with the UTMOS objective.

Same data pipeline and architecture as ../baseline/finetune.py (per-pair score averaging,
repetitive padding, ECAPA + projection + interaction head, AdamW). The variable under test
is --loss:

  mse          plain MSE, exactly what the baseline trains on. The control.
  clipped      clipped MSE alone (tau), isolating the dead zone.
  contrastive  UTMOS contrastive alone. Optimises rank, fixes no absolute scale.
  utmos        beta * clipped + gamma * contrastive -- UTMOS Eq. 1, the headline arm.

See losses.py for the ports and hyperparameter provenance.

Contrastive pairs come from within a micro-batch, so --batch-size matters directly and
--accumulate-steps does NOT substitute for it: a batch of B contributes B*(B-1) ordered
pairs, and accumulation just averages several small pair sets.

Checkpoints are saved as {"config": ..., "state_dict": ...} so inference.py can rebuild the
architecture without being told what it was.
"""
...
from losses import LOSS_CHOICES, CombineLosses, build_loss
...
def main():
    ...
    parser.add_argument("--encoder-source", type=str, default="speechbrain/spkrec-ecapa-voxceleb", help="SpeechBrain EncoderClassifier to fine-tune. Defaults to the baseline's encoder.")
    parser.add_argument("--cache-dir", type=str, default=None, help="Where to cache downloaded encoder weights.")
    parser.add_argument("--loss", type=str, default="utmos", choices=LOSS_CHOICES, help="Training objective. 'mse' reproduces the baseline.")
    parser.add_argument("--tau", type=float, default=0.25, help="Clipped-MSE dead zone (UTMOS shipped value).")
    parser.add_argument("--margin", type=float, default=0.1, help="Contrastive margin (UTMOS shipped value).")
    parser.add_argument("--beta", type=float, default=1.0, help="Weight on the regression term in --loss utmos.")
    parser.add_argument("--gamma", type=float, default=0.5, help="Weight on the contrastive term in --loss utmos.")
    ...
    logging.info(f"Initializing Model for {args.target_metric} with encoder '{args.encoder_source}'...")
    config = {
    ...
    logging.info(
        f"Encoder output dim {model.encoder.output_dim}, "
    ...
    criterion = build_loss(args.loss, tau=args.tau, margin=args.margin,
                           beta=args.beta, gamma=args.gamma).to(device)
    if args.loss == "utmos":
        logging.info(f"Loss: {args.beta} * clipped_mse(tau={args.tau}) "
                     f"+ {args.gamma} * contrastive(margin={args.margin})")
    else:
        logging.info(f"Loss: {args.loss}")
    if args.loss in ("contrastive", "utmos"):
        pairs = args.batch_size * (args.batch_size - 1)
        logging.info(f"Contrastive term sees {pairs} ordered pairs per batch of {args.batch_size}.")
