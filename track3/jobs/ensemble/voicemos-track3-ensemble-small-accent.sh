#!/usr/bin/env bash
#SBATCH --job-name=voicemos-track3-ensemble-small-accent
...
# Ensemble-candidate grid, ACCENT similarity, the two CHEAP encoders.
#
#   sbatch track3/jobs/ensemble/voicemos-track3-ensemble-small-accent.sh
#
...
#   commonaccent-ecapa  bs16x1  0.15 s/step  ->  ~1.0 h per arm
#   eres2netv2          bs4x4   0.42 s/step  ->  ~2.3 h per arm
# So 4 x 1.0 + 4 x 2.3 = ~13.3 h plus inference. Hence 18 h. Cheapest encoder first, so a
# preemption costs the eres2netv2 arms rather than everything.
#
# To split further, or to rerun one cell:
#   sbatch --export=ALL,ARMS="eres2netv2:mse:baseline" --time=04:00:00 ...
#
...
ALL_ARMS="commonaccent-ecapa:mse:baseline commonaccent-ecapa:mse:bilinear \
...
echo "  acc_sim  best single so far   uSRCC 0.547 (w24s4ep4 + coral + moe + baseline)"
echo "  acc_sim  top-8 ensemble       uSRCC 0.579"
echo ""
