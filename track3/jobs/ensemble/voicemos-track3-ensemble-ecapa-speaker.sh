#!/usr/bin/env bash
#SBATCH --job-name=voicemos-track3-ensemble-ecapa-speaker
...
# Ensemble-candidate grid, SPEAKER similarity, the ecapa-voxceleb encoder.
#
#   sbatch track3/jobs/ensemble/voicemos-track3-ensemble-ecapa-speaker.sh
#
# Completes the factorial that the first four jobs left at 3 x 2 x 2. With these four cells
# the candidate pool is a full 4 x 2 x 2 = 16 configurations per target:
#
#   encoder      ecapa-voxceleb | commonaccent-ecapa | eres2netv2 | eres2netv2-w24s4ep4
#   objective    mse | coral
#   interaction  baseline | bilinear
#
# The other 12 cells are already on disk under egs/ensemble_runs/; this job adds the 4
# missing ecapa-voxceleb cells and nothing else.
#
# WHY ecapa-voxceleb. It is the official baseline's encoder, so these cells anchor the grid
# to a published reference point, and it is the one encoder family the first pass omitted.
# In the earlier heterogeneous top-16 ensemble (dev UTT-SRCC 0.607 on spk_sim) five of the
# sixteen members were ecapa-voxceleb runs -- individually the weakest at 0.523-0.548, but
# the ones contributing decorrelated errors. The 12-cell grid, holding four axes fixed,
# produced better average members and a WORSE ensemble (0.587). These four cells test whether
# restoring the missing encoder family recovers part of that gap while keeping the recipe
# uniform -- which is what makes the design reportable as one factorial rather than a list.
#
# Held fixed, identical to the other four jobs: MoE head, freeze 5000/20000,
# --backbone-lr-mult 0.1, 20,000 steps, lr 1e-3, effective batch 16, selection on srcc_utt.
#
# CHECKPOINTS. Written under egs/ensemble_runs/, alongside the existing 24 runs. Two per run:
#
...
# and exactly those two survive.
#
# MEASURED COST. ecapa-voxceleb has the same shape as commonaccent-ecapa (bs 16x1), which ran
# 20,000 steps in 47 min on job 10313630. 4 arms -> ~3.2 h plus inference. Hence 6 h.
#
# To rerun one cell:
#   sbatch --export=ALL,ARMS="ecapa-voxceleb:mse:baseline" --time=02:00:00 ...
#
...
echo "  spk_sim  best single, 12-cell grid   uSRCC 0.569 (w24s4ep4 + mse + baseline)"
echo "  spk_sim  top-12 grid ensemble        uSRCC 0.587"
echo "  spk_sim  heterogeneous top-16        uSRCC 0.607"
echo ""
