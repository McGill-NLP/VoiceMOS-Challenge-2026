#!/usr/bin/env bash
#SBATCH --job-name=voicemos-track3-unified-ixenc-accent
...
# no-b and bilinear interactions on the two ERes2NetV2 encoders, ACCENT similarity.
#
#   sbatch track3/jobs/voicemos-track3-unified-ixenc-accent.sh
#
...
#
#   eres2netv2  + moe + mse + baseline   acc uSRCC 0.487  (job 10298519)
#   w24s4ep4    + moe + mse + baseline   acc uSRCC 0.533  (job 10305054)
#
...
# gain on spk_sim (+0.030 UTT-SRCC) though only +0.002 on acc_sim and bilinear the most consistent across
...
echo "  eres2netv2  baseline  0.360  0.529  0.487   0.040  0.907  0.885       0.465"
echo "  w24s4ep4    baseline  0.348  0.563  0.533   0.045  0.935  0.951       0.522"
echo ""
echo "Best acc_sim on this dev set before this job: uSRCC 0.547, uLCC 0.589, uMSE 0.326"
echo "(w24s4ep4 CORAL); sSRCC 0.970 (base/ecapa ladder); sMSE 0.023 (no-b/ecapa)."
echo ""
