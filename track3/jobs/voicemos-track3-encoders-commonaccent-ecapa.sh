#!/usr/bin/env bash
#SBATCH --job-name=voicemos-track3-encoders-commonaccent-ecapa
...
# Baseline recipe with the CommonAccent ECAPA encoder, for BOTH Track 3 targets.
#
# Jzuluaga/accent-id-commonaccent_ecapa is the same ECAPA architecture as the baseline's,
# fine-tuned on CommonVoice for 16-way English accent ID. The baseline's VoxCeleb encoder
# is trained to be *invariant* to accent, which is why its zero-shot run emits identical
# predictions for spk_sim and acc_sim; this one is discriminative in that dimension. It is
# run on both targets so the speaker/accent trade-off against ERes2NetV2 is measurable.
#
...
#
#   sbatch --export=ALL,USE_LOCAL_SPLITS=1 track3/jobs/voicemos-track3-encoders-commonaccent-ecapa.sh
#
...
#     full fine-tuning  bs 16 -> 11.18 GiB, 0.157 s/step
#     frozen            bs 16 ->  2.96 GiB, 0.068 s/step
BATCH=16
ACCUM=1
...
# only survivable because its encoder is frozen. For real fine-tuning of a 20.8M-param
