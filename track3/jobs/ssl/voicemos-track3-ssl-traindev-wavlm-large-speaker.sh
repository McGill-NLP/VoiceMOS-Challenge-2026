#!/usr/bin/env bash
#SBATCH --job-name=voicemos-track3-ssl-traindev-wavlm-large-speaker
...
# SSL encoder in the DEEP pipeline, SPEAKER similarity, WavLM Large read at layer 4.
#
#   sbatch track3/jobs/ssl/voicemos-track3-ssl-traindev-wavlm-large-speaker.sh
#
...
#
#   encoder      wavlm-large-l4   (fixed in this job)
#   objective    mse | coral
...
#
#   0.404 s/step and 14.8 GiB at batch 16 x 1; 63.5M backbone, layer 4 of 24, dim 1024
#   training               4 arms x ~1.8h = ~7.4h
#   inference              4 arms x 4 passes x ~2 min = ~35 min
#   total                  ~8.1h, hence 11 h with slack.
#
...
#   sbatch --export=ALL,ARMS="wavlm-large-l4:coral:bilinear" --time=03:00:00 \
#       track3/jobs/ssl/voicemos-track3-ssl-traindev-wavlm-large-speaker.sh
#
...
e = build_encoder('wavlm-large-l4')
print('wavlm-large-l4 ready, output_dim', e.output_dim)" \
    || { echo "ERROR: cannot build wavlm-large-l4"; exit 1; }
