#!/usr/bin/env bash
#SBATCH --job-name=voicemos-track3-ssl-layer-probe-xlsr
...
# LAYER PROBE for XLS-R 300M: which transformer layer should the full SSL grid read?
#
#   sbatch track3/jobs/ssl/voicemos-track3-ssl-layer-probe-xlsr.sh
#
...
# l4 0.572, l8 0.542, l24 0.470 on spk_sim; but on acc_sim l8 0.523 BEATS l4
# 0.505, the one place the layer-4 rule already breaks.
#
...
#
#   xlsr-300m-l4      63.5M params    51.7 GFLOP/utt  batch 16 x 1  ~2.1 h  (measured)
#   xlsr-300m-l8     113.9M params    75.6 GFLOP/utt  batch 8 x 2  ~3.1 h  (extrapolated)
#   xlsr-300m-l24    315.4M params   171.5 GFLOP/utt  batch 4 x 4  ~7.1 h  (extrapolated)
#
...
#
#   xlsr-300m-l4   ~2.1 h  (measured, batch 16 x 1)
#   xlsr-300m-l8   ~3.1 h  (extrapolated, batch 8 x 2)
#   xlsr-300m-l24  ~7.1 h  (extrapolated, batch 4 x 4)
#   inference           3 arms x 4 passes x ~2 min = ~25 min
#   total               ~12.8 h, hence 15 h with slack.
#
...
#   sbatch --export=ALL,ARMS="xlsr-300m-l24:mse:baseline" --time=10:00:00 \
#       track3/jobs/ssl/voicemos-track3-ssl-layer-probe-xlsr.sh
#
...
ALL_ARMS="xlsr-300m-l4:mse:baseline xlsr-300m-l8:mse:baseline xlsr-300m-l24:mse:baseline"
ARMS=${ARMS:-"$ALL_ARMS"}
...
echo "=================================================================="
echo "LAYER PROBE, XLS-R 300M, fitted on TRAIN + DEV   metric=$METRIC"
echo "arms: $ARMS"
...
echo ""
echo "=================================================================="
echo "LAYER PROBE RESULT, XLS-R 300M"
echo ""
...
echo "  spk_sim  l4 0.572  l8 0.542  l24 0.470     acc_sim  l4 0.505  l8 0.523  l24 0.375"
echo ""
...
echo "     track3/jobs/ssl/voicemos-track3-ssl-traindev-xlsr-speaker.sh"
