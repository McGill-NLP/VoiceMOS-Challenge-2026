#!/usr/bin/env bash
#SBATCH --job-name=voicemos-track3-strong-traindev-w24-speaker
...
# The strong-learner grid refit on TRAIN + DEV, SPEAKER similarity, the eres2netv2-w24s4ep4 encoder, the slowest of the four.
#
#   sbatch track3/jobs/strong/voicemos-track3-strong-traindev-w24-speaker.sh
#
...
#
# 4 x 2 x 2 = 16 cells per target. This file runs 4 of them for spk_sim;
# voicemos-track3-strong-traindev-small-speaker.sh runs the other 12.
#
...
#
#   eres2netv2-w24s4ep4  4 arms x ~5h00    = ~20h00   (batch 4x4; it OOMs at 16)
#   inference            4 arms x 4 passes x ~3 min = ~50 min
#   total                                   ~21h, which fits in 24 h but leaves little slack.
#   The arm loop is resumable, so a timeout just means resubmitting the same script; splitting
#   it into four ~7 h single-arm jobs with ARMS= usually queues faster.
#
# To run one cell, or to split this job across shorter allocations:
#   sbatch --export=ALL,ARMS="eres2netv2-w24s4ep4:coral:bilinear" --time=07:00:00 \
#       track3/jobs/strong/voicemos-track3-strong-traindev-w24-speaker.sh
#
...
# Grid
#
# One loop over an explicit arm table rather than 4 copies: the arms differ only in
...
echo "  spk_sim  best single, 16-cell grid   uSRCC 0.569 (w24s4ep4 + mse + baseline)"
