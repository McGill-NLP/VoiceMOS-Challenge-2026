#!/usr/bin/env bash
#SBATCH --job-name=voicemos-track3-unified-listener-speaker
...
# Listener-bias modelling for SPEAKER similarity (spk_sim).
#
#   sbatch track3/jobs/voicemos-track3-unified-listener-speaker.sh
#
...
# listeners agree with two others at only SRCC 0.454 on spk_sim, so the reliability of a
# 5-listener mean is ~0.675 and a PERFECT model would score UTT-SRCC ~0.822 against it.
# The current best is 0.572, about 70% of that. Every head, interaction and objective
...
echo ""
echo "Context for spk_sim on this dev set:"
echo "  Baseline 2 published                       uSRCC 0.451"
echo "  best so far (w24s4ep4 + mse + moe)         uSRCC 0.572"
echo "  label-noise ceiling for a perfect model    uSRCC ~0.822"
echo ""
